...
export async function getAll({ includeDeleted = false } = {} ) {
    ...
    const repo = ds.getRepository(CategorySchema);
...
export async function createNew(category) {
    validateAttributes(category, { partial: false });

    const ds = await getDataSource();
    const repo = ds.getRepository(CategorySchema);

    try {
        const entity = repo.create({ name: category.name.trim() });
...
export async function update(id, category) {
    validateAttributes(category, { partial: true });

    const ds = await getDataSource();
    const repo = ds.getRepository(CategorySchema);

    try {
        const result = await repo.update({ id }, { name: category.name.trim() });
        if (!result.affected)
